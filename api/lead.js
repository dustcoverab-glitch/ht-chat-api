...
## RUT- OCH ROT-AVDRAG – KRITISKT VIKTIG INFORMATION
Svara ALLTID korrekt på frågor om skattereduktion. Säg ALDRIG att våra tjänster inte omfattas.

### Stentvätt → RUT-avdrag
Stentvätt är en hushållsnära tjänst och berättigar till RUT-avdrag. Kunden får 30 % skattereduktion på arbetskostnaden. Vi hanterar ansökan smidigt – kunden behöver inte sköta någon administration själv.

### Altantvätt → ROT-avdrag
Altantvätt räknas som underhåll av trä och berättigar till ROT-avdrag. Kunden får 30 % skattereduktion på arbetskostnaden. Vi hanterar ansökan smidigt – kunden behöver inte sköta någon administration själv.

### Asfaltstvätt → RUT-avdrag
Asfaltstvätt är en hushållsnära tjänst och berättigar till RUT-avdrag. Kunden får 30 % skattereduktion på arbetskostnaden. Vi hanterar ansökan smidigt – kunden behöver inte sköta någon administration själv.

### Generellt om RUT/ROT
- Avdraget är 30 % på arbetskostnaden (ej material)
- Gäller privatpersoner – ej företag eller bostadsrättsföreningar
- Vi sköter all administration – kunden ser avdraget tydligt på fakturan
...
10. Säg ALDRIG att stentvätt, altantvätt eller asfaltstvätt inte omfattas av skattereduktion – det gör de. Stentvätt = RUT, altantvätt = ROT, asfaltstvätt = RUT.
...
function hallucGuard(reply, userText) {
  ...
  // ── RUT/ROT – får INTE sägas att det inte gäller ─────────────────────
  const askingTax = /rut|rot|skattereduktion|skatteavdrag|rutavdrag|rotavdrag|30\s*%|hushållsnära/i.test(u);
  const claimsTaxNotApplicable = /(omfattas?\s*inte|gäller\s*inte|ingår\s*inte|har\s*inte\s*rätt|faller\s*inte|berättigar\s*inte)/i.test(r);

  if (askingTax && claimsTaxNotApplicable) {
    // Ta reda på vilken tjänst som efterfrågades för att ge rätt svar
    const asksStone  = /stentvätt|sten/i.test(u);
    const asksAsfalt = /asfalt/i.test(u);
    const asksAltan  = /altan|trall|trädäck/i.test(u);

    let correctInfo = "Ja, våra tjänster omfattas av skattereduktion! Stentvätt och asfaltstvätt berättigar till RUT-avdrag, och altantvätt berättigar till ROT-avdrag. Du får 30 % skattereduktion på arbetskostnaden och vi sköter all administration – du ser avdraget tydligt på fakturan.";

    if (asksStone && !asksAltan && !asksAsfalt) {
      correctInfo = "Ja, stentvätt berättigar till RUT-avdrag! Du får 30 % skattereduktion på arbetskostnaden. Vi sköter all administration – du ser avdraget tydligt på fakturan.";
    } else if (asksAltan && !asksStone && !asksAsfalt) {
      correctInfo = "Ja, altantvätt berättigar till ROT-avdrag! Du får 30 % skattereduktion på arbetskostnaden. Vi sköter all administration – du ser avdraget tydligt på fakturan.";
    } else if (asksAsfalt && !asksStone && !asksAltan) {
      correctInfo = "Ja, asfaltstvätt berättigar till RUT-avdrag! Du får 30 % skattereduktion på arbetskostnaden. Vi sköter all administration – du ser avdraget tydligt på fakturan.";
    }

    return { blocked: true, safe: correctInfo };
  }
