...
async function sendLeadEmail(data) {
  ...
  const html = `
    ...
    <p><strong>Postnummer:</strong> ${esc(data.postal_code || "-")}</p>
    <p><strong>Stad:</strong> ${esc(data.city || "-")}</p>
    <p><strong>Ärende:</strong> ${esc(data.service_interest || "-")}</p>
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: "Ny lead – HT Ytrengöring",
      html,
    }),
  });
